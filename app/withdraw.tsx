...
export default function Withdraw() {
  ...
  const { selectedOperator, withdrawFunds } = useApp();
  ...
  const handleWithdraw = async () => {
    ...
    setLoading(true);
    try {
      const txId = await withdrawFunds(value, selectedOperator);
      router.push({
      ...
      Alert.alert("Withdrawal Error", e.message || "Failed to withdraw funds.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <TopNavBarComponent showBack title="Withdraw to Mobile Money" />
      </View>

      <View style={styles.content}>
        <Text style={styles.label}>Withdraw to Number</Text>
        <View style={styles.detectedRow}>
          <Text style={styles.detectedText}>Registered {selectedOperator} Number</Text>
